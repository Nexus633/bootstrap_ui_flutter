...
void main() {
  ...
  group('BsAlert Tests', () {
    testWidgets('renders child content', (WidgetTester tester) async {
      await tester.pumpWidget(wrap(const BsAlert(child: Text('Alert Message'))));
      expect(find.text('Alert Message'), findsOneWidget);
    });

    testWidgets('dismissible alert shows close icon and can be closed', (WidgetTester tester) async {
      bool closed = false;
    ...
    testWidgets('autoCloseDuration triggers onClose', (WidgetTester tester) async {
      bool closed = false;
      await tester.pumpWidget(wrap(
        BsAlert(
